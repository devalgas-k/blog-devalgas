 ...
 */
public class ArticleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all articles").get("/api/articles").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created article").get("${new_article_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created article").delete("${new_article_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Article entity").exec(scn);
