...
public class FootersGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all footers").get("/api/footers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new footers")
                        .post("/api/footers")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"logoFooters\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_footers_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created footers").get("${new_footers_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created footers").delete("${new_footers_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Footers entity").exec(scn);
