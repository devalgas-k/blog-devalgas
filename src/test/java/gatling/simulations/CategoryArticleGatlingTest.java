...
public class CategoryArticleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all categoryArticles").get("/api/category-articles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new categoryArticle")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"code\": \"SAMPLE_TEXT\"" +
                                ", \"badge\": null" +
                                ", \"descriptionFr\": \"SAMPLE_TEXT\"" +
                                ", \"descriptionEn\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_categoryArticle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created categoryArticle").get("${new_categoryArticle_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created categoryArticle").delete("${new_categoryArticle_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CategoryArticle entity").exec(scn);
