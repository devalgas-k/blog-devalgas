...
public class HeadersGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all headers").get("/api/headers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new headers")
                        .post("/api/headers")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"logoHeaders\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_headers_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created headers").get("${new_headers_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created headers").delete("${new_headers_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Headers entity").exec(scn);
