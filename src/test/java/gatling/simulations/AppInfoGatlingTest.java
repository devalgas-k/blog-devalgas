...
public class AppInfoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all appInfos").get("/api/app-infos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appInfo")
                        .post("/api/app-infos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"keyInfo\": \"SAMPLE_TEXT\"" + ", \"valueInfo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_appInfo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created appInfo").get("${new_appInfo_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created appInfo").delete("${new_appInfo_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppInfo entity").exec(scn);
