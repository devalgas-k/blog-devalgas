...
describe('Subject e2e test', () => {
  ...
  const subjectSample = { titleFr: 'dès que', titleEn: 'tchou tchouu' };
  ...
  describe('new Subject page', () => {
    ...
    it('should create an instance of Subject', () => {
      cy.get(`[data-cy="titleFr"]`).type('aspirer hebdomadaire arrière');
      cy.get(`[data-cy="titleFr"]`).should('have.value', 'aspirer hebdomadaire arrière');

      cy.get(`[data-cy="titleEn"]`).type('de crainte que plouf jusque');
      cy.get(`[data-cy="titleEn"]`).should('have.value', 'de crainte que plouf jusque');
