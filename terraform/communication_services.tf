resource "azurerm_communication_service" "email" {
  count               = var.enable_email_service && var.use_azure_communication_services ? 1 : 0
...
resource "azurerm_email_communication_service_domain" "azure_managed_domain" {
  ...
  email_service_id  = var.use_azure_communication_services ? azurerm_email_communication_service.email_service[0].id : ""
...
resource "azurerm_communication_service_email_domain_association" "assoc" {
  count                    = var.enable_email_service && var.use_azure_communication_services ? 1 : 0
  communication_service_id = var.use_azure_communication_services ? azurerm_communication_service.email[0].id : ""
  email_service_domain_id  = var.use_azure_communication_services ? azurerm_email_communication_service_domain.azure_managed_domain[0].id : ""
}
